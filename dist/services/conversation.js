...
class ConversationService {
    ...
    async getConversationThread(tweetId) {
        try {
            ...
            const { data: mentionTweet, includes } = await x_client_1.xClient.getTweet(tweetId);
            ...
            if (conversationId !== tweetId) {
                try {
                    ...
                    const { data: originalTweet, includes: origIncludes } = await x_client_1.xClient.getTweet(conversationId);
                ...
                for (const ref of mentionTweet.referenced_tweets) {
                    if (ref.type === 'replied_to' && ref.id !== conversationId) {
                        try {
                            ...
                            const { data: parentTweet, includes: parentIncludes } = await x_client_1.xClient.getTweet(ref.id);
            ...
            try {
                ...
                const searchResult = await x_client_1.xClient.searchTweets(query, 100);
     ...
     */
    formatConversation(thread) {
        const lines = ['=== Conversation Thread ==='];
