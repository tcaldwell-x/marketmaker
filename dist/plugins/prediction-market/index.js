 ...
 */
function generateInitialProbability(question) {
        ...
        try {
            switch (toolName) {
                case 'create_market': {
                    ...
                    const resolutionDate = args.resolution_date;
                    const sourceClaim = args.source_claim;
                    // Check for similar existing markets
                    const similar = getExistingMarkets().find(m => m.question.toLowerCase().includes(question.toLowerCase().slice(0, 30)) ||
