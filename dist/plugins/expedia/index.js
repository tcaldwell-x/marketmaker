...
exports.expediaPlugin = void 0;
const expedia_1 = require("../../services/expedia");
/**
...
exports.expediaPlugin = {
    ...
    async initialize(config) {
        ...
        if (apiKey && sharedSecret && !config.sandboxMode) {
            (0, expedia_1.initExpediaClient)(apiKey, sharedSecret, affiliateId, false);
            console.log('[Expedia Plugin] ✅ Using LIVE Expedia API');
        }
        else {
            (0, expedia_1.initExpediaClient)('sandbox', 'sandbox', affiliateId, true);
            console.log('[Expedia Plugin] 🧪 Using SANDBOX mode with simulated data');
        }
    },
    async executeTool(context) {
        const client = (0, expedia_1.getExpediaClient)();
        if (!client) {
        ...
        try {
            switch (toolName) {
                case 'search_hotels': {
                    const hotels = await client.findHotels(args.destination, args.checkin, args.checkout, args.guests || 2);
                    return {
                        success: true,
                        data: {
                            type: 'hotels',
                ...
                case 'search_vacation_rentals': {
                    const rentals = await client.findVacationRentals(args.destination, args.checkin, args.checkout, args.guests || 2, args.bedrooms);
                    return {
                        success: true,
                        data: {
                ...
                case 'search_car_rentals': {
                    const cars = await client.findCarRentals(args.location, args.pickup_date, args.dropoff_date);
                    return {
                        success: true,
                        data: {
                            type: 'car_rentals',
                ...
                case 'search_activities': {
                    const activities = await client.findActivities(args.destination, args.start_date, args.end_date, args.travelers || 2);
                    return {
                        success: true,
                        data: {
                            type: 'activities',
    ...
    extractStorableData(toolResults, _grokMessage) {
        let destination = '';
        let hotel = null;
        let activity = null;
        ...
        for (const result of toolResults) {
            ...
            if (data.type === 'activities' && data.items.length > 0) {
                ...
                if (!hotel)
                    actionUrl = activity.booking_url;
            }
        }
        if (!hotel && !activity)
            return null;
...
exports.default = exports.expediaPlugin;
